...
function ActiveQuestionCard({
  ...
  function handleAnswer(index) {
    ...
    const nextState = {
      ...state,
      totalPoints: totalPoints + earned,
      answersStatus: [...answersStatus, correct],
      usedCounts: {
        ...usedCounts,
    ...
    setTimeout(() => {
      if (nextTotalAnswered >= totalQuestions) {
        navigate("/student/final-results", { state: nextState });
        return;
      }

      navigate("/student/difficulty", { state: nextState });
    }, 900);
  }

  const submitTimedOutAnswer = useEffectEvent(() => {
    handleAnswer(null);
  });
...
function Question() {
  ...
  const list = useMemo(
    () => questionsByDifficulty[difficulty] ?? questionsByDifficulty.medium ?? [],
    [difficulty, questionsByDifficulty]
  );
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center px-6">
      <div className="w-full max-w-3xl bg-slate-800 border border-slate-600 rounded-2xl shadow-xl p-8">
        ...
        <ActiveQuestionCard
          key={`${difficulty}-${localIndex}-${timePerQuestion}`}
